...
import { SCIENCE_OPERATIONS } from '@/lib/tools/science-education-workbench';
...
/*
  ...
  Every tool under /science used to answer on a single workbench URL behind a
  ...
  at `/science/<id>`, each with its own title, description and canonical
...
const OPERATIONS = [
  ...new Map(
    [SCIENCE_OPERATIONS].flat().map((operation) => [operation.id, operation]),
  ).values(),
];
...
export default async function Page({
  ...
  if (SCIENCE_OPERATIONS.some((operation) => operation.id === tool)) {
    return (
