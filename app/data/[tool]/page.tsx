...
import { SPREADSHEET_OPERATIONS } from '@/lib/tools/spreadsheet-workbench';
...
/*
  ...
  Every tool under /data used to answer on a single workbench URL behind a
  ...
  at `/data/<id>`, each with its own title, description and canonical
...
const DEDICATED = new Set<string>(['csv-to-json']);
...
export default async function Page({
  ...
  const { tool } = await params;
  if (SPREADSHEET_OPERATIONS.some((operation) => operation.id === tool)) {
    return (
