...
/*
  The localised editions of `/image/optimize`, one static page per locale.

  ONE FILE, NOT ONE PER LANGUAGE. `generateStaticParams` returns the eight
  locale codes, so this single route file produces `/es/image/optimize`,
  `/ja/image/optimize` and the other six, each prerendered with its own title,
  ...
  segment beats a dynamic one in the router, so the English `/image/optimize` is
