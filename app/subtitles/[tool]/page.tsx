...
import { SUBTITLE_OPERATIONS } from '@/lib/tools/subtitle-workbench';
...
/*
  ...
  Every tool under /subtitles used to answer on a single workbench URL behind a
  ...
  at `/subtitles/<id>`, each with its own title, description and canonical
...
const OPERATIONS = [
  ...new Map(
    [SUBTITLE_OPERATIONS].flat().map((operation) => [operation.id, operation]),
  ).values(),
];
...
export default async function Page({
  ...
  const { tool } = await params;
  if (SUBTITLE_OPERATIONS.some((operation) => operation.id === tool)) {
    return (
      <SubtitleWorkbenchTool initialOperationId={tool} routedBasePath={BASE} />
    );
  }
  return null;
}
