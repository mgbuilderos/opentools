import type { Schema, SchemaColumn } from './types';
  ...
  for (const table of schema.tables) {
    ...
    for (const col of table.columns) {
      ...
      if (
        ...
        !attrs.some((a) => a.includes('@default'))
      ) {
    ...
    // Add relation fields
    const childRelations = schema.relations.filter(
      (r) => r.fromTable.toLowerCase() === table.name.toLowerCase(),
    );
    ...
    const parentRelations = schema.relations.filter(
      (r) => r.toTable.toLowerCase() === table.name.toLowerCase(),
    );
...
function mapTypeToDjango(col: SchemaColumn): string {
  ...
  if (norm.includes('bool'))
    return `models.BooleanField(default=False${nullOpt})`;
  if (norm.includes('int'))
    return `models.IntegerField(${nullOpt.slice(2)}${uniqOpt})`;
  if (
    norm.includes('decimal') ||
    norm.includes('float') ||
    norm.includes('double')
  ...
  if (norm.includes('text'))
    return `models.TextField(${nullOpt.slice(2)}${uniqOpt})`;
