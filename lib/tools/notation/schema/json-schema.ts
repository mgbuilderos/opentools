...
export function parseJsonSchema(input: string): Schema {
  ...
  const definitions = (doc.definitions || doc.$defs || {}) as Record<
    string,
  ...
  for (const [tableName, def] of Object.entries(definitions)) {
    ...
    for (const [colName, p] of Object.entries(props)) {
      ...
      const jsonType = typeof rawType === 'string' ? rawType : 'string';
      ...
      const commentStr = typeof rawDesc === 'string' ? rawDesc : undefined;
    ...
    const tableComment = typeof defDesc === 'string' ? defDesc : undefined;
