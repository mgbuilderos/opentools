...
export const PAGE_DEPTH_CORE: Readonly<Record<string, ToolPageDepth>> = {
  ...
  // components/video-trim-tool.tsx, lib/tools/video/
  '/video/trim': {
    title: 'Trim Video Online — Lossless MP4 Cutter, No Upload',
    description:
      'Trim MP4 videos locally in your browser. Cuts on keyframes without re-encoding, preserving 100% original video and audio quality. No upload.',
    heading: 'About this video trimmer',
    directAnswer:
      'Open an MP4 or M4V video file up to 200 MB, set your start and end cut points using the interactive scrubber, and click Trim Video. The video is cut on keyframe boundaries without re-encoding, preserving exact original video resolution, audio bitrates, and color metadata with zero quality loss.',
    ...
    steps: [
      {
        name: 'Select MP4 video',
        text: 'Choose an MP4 or M4V video file up to 200 MB. The video loads into local memory and displays in the video preview player.',
      },
      {
        name: 'Set cut interval',
        text: 'Use the interactive timeline scrubber or type exact start and end timestamps (H:MM:SS.ms) to mark the trim range.',
      },
      {
        name: 'Execute keyframe-aligned cut',
        text: 'Click Trim Video. The demuxer slices video and audio samples directly without re-encoding, packaging them into a new container.',
      },
      {
        name: 'Preview and download',
        text: 'Play the trimmed clip directly in the preview player to verify seamless playback, then download the trimmed MP4 directly to your drive.',
      },
    ],
    sections: [
      {
        ...
        body: [
          'Video compression groups frames into GOPs (Group of Pictures) starting with an independent keyframe (I-frame / IDR frame) followed by predicted frames (P-frames and B-frames). Re-encoding video decompresses every frame and compresses it again, causing visible compression artifacts, color shifts, and high CPU usage. By copying compressed bitstream packets directly and cutting at keyframes, this tool outputs video with zero loss in visual fidelity at the speed of your local storage drive.',
        ],
      },
      {
        heading: 'MP4 container compatibility and limits',
        body: [
          'The trimmer supports MP4 and M4V video containers holding H.264 (AVC) or H.265 (HEVC) video alongside AAC or MP3 audio. Files with complex edit lists or fragmented tracks are normalized on export. Maximum input file size is set to 200 MB to ensure safe execution within browser tab memory allocations.',
        ],
      },
      {
        heading: 'Atom box parsing in the ISO Base Media format',
        body: [
          'MP4 files structure media information into hierarchical atom boxes (ftyp, moov, mdat, mvhd, trak, mdia, minf, stbl). The trimmer parses the sample table box (stbl) to locate the sync sample table (stss), which identifies the exact temporal positions of every keyframe in the video stream, ensuring clean cut points.',
        ],
      },
      {
        heading: 'Zero upload security guarantee',
    ...
    faqs: [
      ...
      {
        question: 'Why are cut points aligned to keyframes?',
        answer:
          'In compressed video, predicted frames rely on preceding keyframes to render. Cutting between keyframes without re-encoding causes visual corruption (macroblocking); aligning cuts to keyframes guarantees clean playback.',
      },
      {
        question: 'Which video formats are supported?',
        answer:
          'Standard MP4 and M4V files with H.264/AVC or H.265/HEVC video and AAC audio are supported. MKV or AVI containers should be converted to MP4 before trimming.',
      },
      {
      ...
      {
        question: 'What is the maximum video file size permitted?',
        answer:
          'Video files up to 200 MB are supported for in-browser memory manipulation without exhausting browser tab allocations.',
      },
    ],
  },
};
