...
describe('kernel manifest', () => {
  ...
  it('registers every measured workbench operation once per source', () => {
    expect(KERNEL_MANIFEST).toHaveLength(637);
    expect(new Set(KERNEL_MANIFEST.map(descriptorKey)).size).toBe(637);
    expect(new Set(KERNEL_MANIFEST.map((item) => item.source)).size).toBe(19);
  });
});
