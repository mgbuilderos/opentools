...
if (urls.length === 0) {
  console.error('Empty sitemap — refusing to report a clean sweep of nothing.');
  process.exit(2);
}
  ...
  if (!findings.length) console.log('  0 faults.\n');
    ...
    for (const [check, list] of [...by].sort(
      ...
      for (const f of list.slice(0, 4))
        console.log(
          `        - ${f.url.replace(ORIGIN, '') || '/'} : ${f.detail}`,
        );
      if (list.length > 4)
        console.log(`        ... and ${list.length - 4} more`);
      console.log('');
    }
